...
const addFriend = async (user) => {
    try {
        ...
        console.log('code 为  :', response.code);
        // 检查后端返回的状态
        if (response.code === 1) {  // 后端成功时返回 code 1
            ...
            // 如果后端返回的不是成功状态，显示后端返回的消息
            ElMessage.warning(response.msg || "添加好友失败，请稍后再试。");
        }
    } catch (error) {
                    ...
                    <el-scrollbar style="max-height: 300px; overflow-y: auto;">
                        <template v-if="searchList.length > 0">
                            <div v-for="user in searchList" :key="user.id" class="search-result-item">
                                ...
                                <el-button type="primary" size="mini" @click="addFriend(user)">添加好友</el-button>
                            </div>
                        </template>
                        <template v-else>
                            <p>没有找到相关用户。</p>
            ...
            <el-main class="chat-main">
                ...
                "
                    ...
                    <div class="chat-input">
                        ...
                        >
                            <template #prepend>
                                <el-button type="text" @click="showDialog = !showDialog"
